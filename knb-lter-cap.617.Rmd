...
microbial <- read_csv("data/Microbial.csv") %>%
  ...
  mutate(cluster = tolower(cluster),
         ...
         `site number` = as.factor(`site number`), 
         `site acronym` = as.factor(`site acronym`),
         `E coli(colonies per 100mL)` = as.numeric(`E coli(colonies per 100mL)`),
         `Coliform(colonies per 100mL)` = as.numeric(`Coliform(colonies per 100mL)`),
         `Mycobacterium(colonies per 100mL)` = as.numeric(`Mycobacterium(colonies per 100mL)`),
         date = format(as.POSIXct(`date`, format = "%m/%d/%y"), "%Y-%m-%d"),
         date = as.Date(date)  ) %>%
  select(-`ID`) %>%
  arrange(date, `site number`) %>% 
...
sampling_locations$longitude <- as.numeric(as.character(sampling_locations$longitude))
...
sampling_locations_DT <- create_dataTable(dfname = sampling_locations,
