...
dplyr::glimpse(algal_byproducts)
...
carbon <- readr::read_csv("data/doc_month.csv") |>
  dplyr::rename(`cluster` = `cluster name`) |>
  dplyr::mutate(
    cluster        = tolower(cluster),
    cluster        = as.factor(cluster),
...
dplyr::glimpse(field_measurements)
...
dplyr::glimpse(quarterly_lake_sampling)
...
dplyr::glimpse(quarterly_metals)
...
dplyr::glimpse(sampling_locations)
...
peter   <- gioseml::create_role(firstName = "peter", lastName = "fox", roleType = "creator")
morteza <- gioseml::create_role(firstName = "m", lastName = "abbaszadegan", roleType = "creator")

creators <- list(peter, morteza)

marisa <- gioseml::create_role(firstName = "m", lastName = "masles", roleType = "metadata")
metadataProvider <- list(marisa)

paul <- gioseml::create_role(firstName = "p", lastName = "westerhoff", roleType = "associated")
milt <- gioseml::create_role(firstName = "m", lastName = "sommerfeld", roleType = "associated")
...
coverage <- EML::set_coverage(
  ...
  west                  = min(sampling_locations[["longitude"]], na.rm = TRUE),
  east                  = max(sampling_locations[["longitude"]], na.rm = TRUE),
  north                 = max(sampling_locations[["latitude"]], na.rm = TRUE),
  south                 = min(sampling_locations[["latitude"]], na.rm = TRUE)
)
